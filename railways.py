...
API_KEY='fad77d39-5543-4e77-9eeb-2ca47518649d'
...
def get_trains_bw_stations(json_trains):
    trains_list=[]
    id_=1
    for x in json_trains['result'][:10]:
        tmp=[]
        tmp.append(id_)
        id_+=1
...
def get_availabilty(train_no,source_code,dest_code,date,API_KEY,quota='GN'):
        print('Enter Class(SL|3A|2A|1A)')
        class_=raw_input()
        url='http://api.erail.in/seats/?key='+API_KEY
        url+='&trainno='+train_no
        url+='&stnfrom='+source_code+'&stnto='+dest_code
        url+='&quota=GN'+class_+'&date='+date
...
def display_trains_and_get_details(trains_list,date,API_KEY):
    print('Enter serial no. to get details')
    ...
    print('Enter 1 to get seat availabilty')
    choice=raw_input()
    if choice=='1':
        print(get_availabilty(train_no,source_code,dest_code,date,API_KEY))
